...
<style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .container > #configCard { grid-column: 1; grid-row: 2; }
            ...
            @media (max-width: 900px) {
                ...
                .container > .header, .container > #configCard, .container > .right-column { grid-column: 1; grid-row: auto; }
            }
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="card panel-right-card">
                ...
                <div id="latencyTestSection" style="display:none;margin-top:14px;border-top:1px solid rgba(0,240,255,.15);padding-top:12px;">
                        <div style="margin-bottom: 20px; padding: 15px; background: transparent; border: 2px solid #7aa9c4; border-radius: 8px;">
                            ...
                            <div style="display: flex; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; align-items: center;">
                                ...
                                <div style="width: 80px;">
                                    <label style="display: block; margin-bottom: 5px; color: #00f0ff; font-size: 0.9rem;">${语言代码236 === 'fa' ? 'رشته‌ها' : 语言代码236 === 'en' ? 'Threads' : '线程'}</label>
                            ...
                            <div id="latencyTestResults" style="max-height: 250px; overflow-y: auto; display: none;">
                                ...
                                <div id="cityFilterContainer" style="margin-bottom: 10px; padding: 10px; background: rgba(15, 3, 40, 0.6); border: 1px solid #7aa9c4; border-radius: 4px; display: none;">
                                    <div style="margin-bottom: 8px;">
                                        <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff; font-size: 0.9rem;">
                                            ...
                                            <span>${语言代码236 === 'fa' ? '全部城市' : 语言代码236 === 'en' ? 'All cities' : '全部城市'}</span>
                                        </label>
                                        <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff; font-size: 0.9rem; margin-left: 15px;">
                                            ...
                                            <span>${语言代码236 === 'fa' ? '只选择最快的10个' : 语言代码236 === 'en' ? 'Select only the fastest 10' : '只选择最快的10个'}</span>
                                        </label>
                                    </div>
                                ...
                                <div style="margin-top: 10px; display: flex; gap: 10px;">
                                    <button type="button" id="overwriteSelectedToYx" style="flex: 1; background: rgba(0, 220, 130, 0.3); border: 1px solid #00f0ff; padding: 10px 20px; color: #00f0ff; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer; transition: all 0.3s;">${语言代码236 === 'fa' ? '覆盖添加' : 语言代码236 === 'en' ? 'Overwrite add' : '覆盖添加'}</button>
                                    <button type="button" id="appendSelectedToYx" style="flex: 1; background: rgba(0, 178, 110, 0.3); border: 1px solid #7aa9c4; padding: 10px 20px; color: #7aa9c4; font-family: 'Courier New', monospace; font-weight: bold; cursor: pointer; transition: all 0.3s;">${语言代码236 === 'fa' ? '追加添加' : 语言代码236 === 'en' ? 'Append add' : '追加添加'}</button>
                                </div>
                            </div>
                        </div>
                </div>
                ...
                <div id="configContent" style="display: none;">
                    ...
                    <form id="otherConfigForm" style="margin-bottom: 20px;">
                        ...
                        <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" id="customPath" placeholder="${语言代码236 === 'fa' ? 'مثال: /mypath یا خالی بگذارید تا از UUID استفاده شود' : 语言代码236 === 'en' ? 'e.g. /mypath or leave empty to use UUID' : '例如: /mypath 或留空使用 UUID'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${语言代码236 === 'fa' ? "مسیر اشتراک سفارشی. اگر خالی بگذارید از UUID به عنوان مسیر استفاده می‌شود." : 语言代码236 === 'en' ? "Custom subscription path. Leave empty to use UUID as the path." : "自定义订阅路径。留空则使用 UUID 作为路径。"}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                                <label style="display: block; margin-bottom: 8px; color: #00f0ff; font-weight: bold; text-shadow: 0 0 3px #00f0ff;">${翻译值.customIP}</label>
                                <input type="text" id="customIP" placeholder="${语言代码236 === 'fa' ? 'مثال: 1.2.3.4:443' : 语言代码236 === 'en' ? 'e.g. 1.2.3.4:443' : '例如: 1.2.3.4:443'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${语言代码236 === 'fa' ? "آدرس و پورت ProxyIP سفارشی" : 语言代码236 === 'en' ? "Custom ProxyIP address and port" : "自定义ProxyIP地址和端口"}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" id="yx" placeholder="${语言代码236 === 'fa' ? 'مثال: 1.2.3.4:443#گره هنگ‌کنگ,5.6.7.8:80#گره آمریکا,example.com:8443#گره سنگاپور' : 语言代码236 === 'en' ? 'e.g. 1.2.3.4:443#JP node,5.6.7.8:80#US node,example.com:8443#SG node' : '例如: 1.2.3.4:443#日本节点,5.6.7.8:80#美国节点,example.com:8443#新加坡节点'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${语言代码236 === 'fa' ? 'فرمت: IP:پورت#نام گره یا IP:پورت (بدون # از نام پیش‌فرض استفاده می‌شود). پشتیبانی از چندین مورد، با کاما جدا می‌شوند. <span style="color: #ffb400;">IP های اضافه شده از طریق API به طور خودکار در اینجا نمایش داده می‌شوند.</span>' : 语言代码236 === 'en' ? 'Format: IP:port#node-name or IP:port (no # = default name). Multiple entries supported, separated by commas.<span style="color: #ffb400;">IPs added via API are shown here automatically.</span>' : '格式: IP:端口#节点名称 或 IP:端口 (无#则使用默认名称)。支持多个，用逗号分隔。<span style="color: #ffb400;">API添加的IP会自动显示在这里。</span>'}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" id="yxURL" placeholder="${语言代码236 === 'fa' ? 'URL منبع لیست IP ترجیحی را وارد کنید' : 语言代码236 === 'en' ? 'Enter preferred IP list source URL' : '输入优选IP列表来源URL'}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${语言代码236 === 'fa' ? 'URL منبع لیست IP ترجیحی سفارشی، اگر خالی بگذارید از آدرس پیش‌فرض استفاده می‌شود' : 语言代码236 === 'en' ? 'Custom preferred IP list source URL; leave empty to use the default address' : '自定义优选IP列表来源URL，留空则使用默认地址'}</small>
                        </div>
                        ...
                        <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" id="socksConfig" placeholder="${语言代码236 === 'fa' ? "مثال: user:pass@host:port یا http://user:pass@host:port" : 语言代码236 === 'en' ? "e.g. user:pass@host:port or http://user:pass@host:port" : "例如: user:pass@host:port 或 http://user:pass@host:port"}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${语言代码236 === 'fa' ? "آدرس پروکسی خروجی برای انتقال تمام ترافیک خروجی. بدون پیشوند به صورت s5 در نظر گرفته می‌شود" : 语言代码236 === 'en' ? "Outbound proxy address for forwarding all outbound traffic. Without a prefix it is treated as s5" : "出站代理地址，用于转发所有出站流量。不写前缀默认按 s5 处理"}</small>
                        </div>
                    </form>
